...
import at.fh.ima.swengs.beadoc.model.*;
...
public class UserDetailsServiceImpl implements UserDetailsService {
  ...
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    try {
      at.fh.ima.swengs.beadoc.model.User user = userRepository.findByUsername(username);
  ...
  public void initUsers() {
    if (userRepository.count() == 0) {
      at.fh.ima.swengs.beadoc.model.User admin = new at.fh.ima.swengs.beadoc.model.User();
      admin.setUsername("admin");
      admin.setPassword(encoder.encode("12345"));
      admin.setAdmin(true);
      userRepository.save(admin);

      at.fh.ima.swengs.beadoc.model.User tester = new at.fh.ima.swengs.beadoc.model.User();
    ...
    if (bloodTypeRepository.count() == 0) {
      List<String> bloodTypeNames = Arrays.asList("A", "B", "AB", "O");
      List<BloodType> bloodTypes = new ArrayList<>();
      bloodTypeNames.forEach(bloodTypeName -> {
        BloodType bloodType = new BloodType();
        bloodType.setName(bloodTypeName);
